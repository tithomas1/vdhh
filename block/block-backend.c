...
static VeertuAioContext *blk_aiocb_get_aio_context(BlockAIOCB *acb);
...
int blk_write_zeroes(BlockBackend *blk, int64_t sector_num, int nb_sectors, BdrvRequestFlags flags)
{
    return bdrv_write_zeroes(blk->bs, sector_num, nb_sectors, flags);
}

int blk_write_compressed(BlockBackend *blk, int64_t sector_num, const uint8_t *buf, int nb_sectors)
{
...
{
    return vmx_blockalign(blk ? blk->bs : NULL, size);
}
...
VeertuAioContext *blk_get_aio_context(BlockBackend *blk)
{
...
void blk_set_aio_context(BlockBackend *blk, VeertuAioContext *new_context)
{
...
void *blk_aio_get(const AIOCBInfo *aiocb_info, BlockBackend *blk,
                  BlockCompletionFunc *cb, void *opaque)
{
    return vmx_aio_get(aiocb_info, blk_bs(blk), cb, opaque);
}
