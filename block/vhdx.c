                             ...
                             uint64_t offset, bool read)
{
    ...
    buffer = vmx_blockalign(bs_file, VHDX_HEADER_SIZE);
    if (read) {
                              ...
                              Error **errp)
{
    ...
    header1 = vmx_blockalign(bs, sizeof(VHDXHeader));
    header2 = vmx_blockalign(bs, sizeof(VHDXHeader));

    buffer = vmx_blockalign(bs, VHDX_HEADER_SIZE);
...
static int vhdx_open_region_tables(BlockDriverState *bs, BDRVVHDXState *s)
{
     ...
     * whole block */
    buffer = vmx_blockalign(bs, VHDX_HEADER_BLOCK_SIZE);
...
static int vhdx_parse_metadata(BlockDriverState *bs, BDRVVHDXState *s)
{
    ...
    buffer = vmx_blockalign(bs, VHDX_METADATA_TABLE_MAX_SIZE);
...
static void vhdx_close(BlockDriverState *bs)
{
    ...
    vmx_vfree(s->headers[0]);
    s->headers[0] = NULL;
    vmx_vfree(s->headers[1]);
    s->headers[1] = NULL;
    vmx_vfree(s->bat);
    s->bat = NULL;
    vmx_vfree(s->parent_entries);
    s->parent_entries = NULL;
    savevm_del_blocker(s->migration_blocker);
                     ...
                     Error **errp)
{
    ...
    s->bat = vmx_try_blockalign(bs->file, s->bat_rt.length);
    if (s->bat == NULL) {
        ret = -ENOMEM;
        goto fail;
    }
    ...
    //savevm_add_blocker(s->migration_blocker);
                                      ...
                                      int nb_sectors, QEMUIOVector *qiov)
{
    ...
    while (nb_sectors > 0) {
        ...
        if (s->params.data_bits & VHDX_PARAMS_HAS_PARENT) {
            ...
            vmx_iovec_concat(&hd_qiov, qiov,  bytes_done, sinfo.bytes_avail);
            ...
            case PAYLOAD_BLOCK_ZERO:
                /* return zero */
                vmx_iovec_memset(&hd_qiov, 0, 0, sinfo.bytes_avail);
                break;
            ...
            case PAYLOAD_BLOCK_ZERO:
                ...
                if (bdrv_has_zero_init(bs->file) == 0) {
                    ...
                    if (sinfo.block_offset) {
                        ...
                        iov1.iov_base = vmx_blockalign(bs, iov1.iov_len);
                        memset(iov1.iov_base, 0, iov1.iov_len);
                        vmx_iovec_concat_iov(&hd_qiov, &iov1, 1, 0,
                                              sinfo.block_offset);
                    ...
                    if ((sinfo.bytes_avail - sinfo.block_offset) <
                        ...
                        iov2.iov_base = vmx_blockalign(bs, iov2.iov_len);
                        memset(iov2.iov_base, 0, iov2.iov_len);
                        vmx_iovec_concat_iov(&hd_qiov, &iov2, 1, 0,
                                              sinfo.block_offset);
...
exit:
    vmx_vfree(iov1.iov_base);
    vmx_vfree(iov2.iov_base);
    vmx_co_mutex_unlock(&s->lock);
    vmx_iovec_destroy(&hd_qiov);
    return ret;
}
...
static int vhdx_create(const char *filename, QemuOpts *opts, Error **errp)
{
    ...
    image_size = ROUND_UP(vmx_opt_get_size_del(opts, BLOCK_OPT_SIZE, 0),
                          BDRV_SECTOR_SIZE);
    log_size = vmx_opt_get_size_del(opts, VHDX_BLOCK_OPT_LOG_SIZE, 0);
    block_size = vmx_opt_get_size_del(opts, VHDX_BLOCK_OPT_BLOCK_SIZE, 0);
    type = vmx_opt_get_del(opts, BLOCK_OPT_SUBFMT);
    use_zero_blocks = vmx_opt_get_bool_del(opts, VHDX_BLOCK_OPT_ZERO, true);
    ...
    creator = g_utf8_to_utf16("Veertu", -1, NULL,
                              &creator_items, NULL);
