...
{
    return (uint32_t)rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].limit);
}

uint32_t vmx_read_segment_ar(CPUState *cpu, x86_reg_segment seg)
{
    return (uint32_t)rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].ar_bytes);
}

uint64_t vmx_read_segment_base(CPUState *cpu, x86_reg_segment seg)
{
    return rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].base);
}

x68_segment_selector vmx_read_segment_selector(CPUState *cpu, x86_reg_segment seg)
{
    x68_segment_selector sel;
    sel.sel = rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].selector);
    return sel;
}

void vmx_write_segment_selector(struct CPUState *cpu, x68_segment_selector selector, x86_reg_segment seg)
{
    wvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].selector, selector.sel);
}

void vmx_read_segment_descriptor(struct CPUState *cpu, struct vmx_segment *desc, x86_reg_segment seg)
{
    desc->sel = rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].selector);
    desc->base = rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].base);
    desc->limit = rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].limit);
    desc->ar = rvmcs(cpu->mac_vcpu_fd, vmx_segment_fields[seg].ar_bytes);
}
...
void vmx_write_segment_descriptor(CPUState *cpu, struct vmx_segment *desc, x86_reg_segment seg)
{
    ...
    wvmcs(cpu->mac_vcpu_fd, sf->base, desc->base);
    wvmcs(cpu->mac_vcpu_fd, sf->limit, desc->limit);
    wvmcs(cpu->mac_vcpu_fd, sf->selector, desc->sel);
    wvmcs(cpu->mac_vcpu_fd, sf->ar_bytes, desc->ar);
}
