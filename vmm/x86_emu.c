...
void veertu_handle_io(struct CPUState *cpu, uint16_t port, void *data, int direction, int size, uint32_t count);
...
addr_t read_reg(struct CPUState* cpu, int reg, int size)
{
    switch (size) {
        case 1:
            return cpu->regs[reg].lx;
        case 2:
            return cpu->regs[reg].rx;
        case 4:
            return cpu->regs[reg].erx;
        case 8:
            return cpu->regs[reg].rrx;
        default:
...
void write_reg(struct CPUState* cpu, int reg, addr_t val, int size)
{
    switch (size) {
        ...
        case 2:
            ...
            break;
        case 4:
            cpu->regs[reg].rrx = (uint32_t)val;
            break;
        case 8:
            cpu->regs[reg].rrx = val;
            break;
        default:
...
{
    if (ptr > (addr_t)cpu && ptr < (addr_t)cpu + sizeof(struct CPUState))  {
...
{
    vmx_read_mem(cpu, cpu->mmio_buf, ptr, bytes);
    return cpu->mmio_buf;
}
...
static void exec_out(struct CPUState *cpu, struct x86_decode *decode)
{
    switch (decode->opcode[0]) {
        case 0xe6:
            veertu_handle_io(cpu, decode->op[0].val, &AL(cpu), 1, 1, 1);
            break;
        case 0xe7:
            veertu_handle_io(cpu, decode->op[0].val, &RAX(cpu), 1, decode->operand_size, 1);
            break;
        case 0xee:
            veertu_handle_io(cpu, DX(cpu), &AL(cpu), 1, 1, 1);
            break;
        case 0xef:
            veertu_handle_io(cpu, DX(cpu), &RAX(cpu), 1, decode->operand_size, 1);
            break;
        default:
...
static void exec_in(struct CPUState *cpu, struct x86_decode *decode)
{
    ...
    switch (decode->opcode[0]) {
        case 0xe4:
            veertu_handle_io(cpu, decode->op[0].val, &AL(cpu), 0, 1, 1);
            break;
        case 0xe5:
            veertu_handle_io(cpu, decode->op[0].val, &val, 0, decode->operand_size, 1);
            ...
            break;
        case 0xec:
            veertu_handle_io(cpu, DX(cpu), &AL(cpu), 0, 1, 1);
            break;
        case 0xed:
            veertu_handle_io(cpu, DX(cpu), &val, 0, decode->operand_size, 1);
            if (decode->operand_size == 2)
                AX(cpu) = val;
            else
                RAX(cpu) = (uint32_t)val;
...
static void exec_ins_single(struct CPUState *cpu, struct x86_decode *decode)
{
    ...
    veertu_handle_io(cpu, DX(cpu), cpu->mmio_buf, 0, decode->operand_size, 1);
    vmx_write_mem(cpu, addr, cpu->mmio_buf, decode->operand_size);
...
static void exec_outs_single(struct CPUState *cpu, struct x86_decode *decode)
{
    ...
    vmx_read_mem(cpu, cpu->mmio_buf, addr, decode->operand_size);
    veertu_handle_io(cpu, DX(cpu), cpu->mmio_buf, 1, decode->operand_size, 1);
...
void simulate_rdmsr(struct CPUState *cpu)
{
    ...
    switch (msr) {
        case MSR_IA32_TSC:
            val = rdtscp() + rvmcs(cpu->mac_vcpu_fd, VMCS_TSC_OFFSET);
            break;
        ...
        case MSR_IA32_UCODE_REV:
            ...
            break;
        case MSR_EFER:
            val = rvmcs(cpu->mac_vcpu_fd, VMCS_GUEST_IA32_EFER);
            break;
        case MSR_FSBASE:
            val = rvmcs(cpu->mac_vcpu_fd, VMCS_GUEST_FS_BASE);
            break;
        case MSR_GSBASE:
            val = rvmcs(cpu->mac_vcpu_fd, VMCS_GUEST_GS_BASE);
            break;
        case MSR_KERNELGSBASE:
            val = rvmcs(cpu->mac_vcpu_fd, VMCS_HOST_FS_BASE);
            break;
        case MSR_STAR:
            abort();
            break;
    ...
    if (g_hypervisor_iface)
        g_hypervisor_iface->rdmsr_handler(cpu, msr);
}
...
void simulate_wrmsr(struct CPUState *cpu)
{
    ...
    switch (msr) {
        case MSR_IA32_TSC:
            if (!osx_is_sierra())
                wvmcs(cpu->mac_vcpu_fd, VMCS_TSC_OFFSET, data - rdtscp());
            //hv_vm_sync_tsc(data);
            break;
        case MSR_IA32_APICBASE:
            printf("MSR_IA32_APICBASE %llx\n", data);
            cpu_set_apic_base(X86_CPU(cpu)->apic_state, data);
            break;
        case MSR_FSBASE:
            wvmcs(cpu->mac_vcpu_fd, VMCS_GUEST_FS_BASE, data);
            break;
        case MSR_GSBASE:
            wvmcs(cpu->mac_vcpu_fd, VMCS_GUEST_GS_BASE, data);
            break;
        case MSR_KERNELGSBASE:
            wvmcs(cpu->mac_vcpu_fd, VMCS_HOST_FS_BASE, data);
            break;
        case MSR_STAR:
            abort();
            break;
        ...
        case MSR_EFER:
            cpu->efer.efer = data;
            //printf("new efer %llx\n", EFER(cpu));
            wvmcs(cpu->mac_vcpu_fd, VMCS_GUEST_IA32_EFER, data);
            if (data & EFER_NXE)
...
{
    int i = 0;
    RRX(cpu, REG_RAX) = rreg(cpu->mac_vcpu_fd, HV_X86_RAX);
    RRX(cpu, REG_RBX) = rreg(cpu->mac_vcpu_fd, HV_X86_RBX);
    RRX(cpu, REG_RCX) = rreg(cpu->mac_vcpu_fd, HV_X86_RCX);
    RRX(cpu, REG_RDX) = rreg(cpu->mac_vcpu_fd, HV_X86_RDX);
    RRX(cpu, REG_RSI) = rreg(cpu->mac_vcpu_fd, HV_X86_RSI);
    RRX(cpu, REG_RDI) = rreg(cpu->mac_vcpu_fd, HV_X86_RDI);
    RRX(cpu, REG_RSP) = rreg(cpu->mac_vcpu_fd, HV_X86_RSP);
    RRX(cpu, REG_RBP) = rreg(cpu->mac_vcpu_fd, HV_X86_RBP);
    for (i = 8; i < 16; i++)
        RRX(cpu, i) = rreg(cpu->mac_vcpu_fd, HV_X86_RAX + i);
    
    RFLAGS(cpu) = rreg(cpu->mac_vcpu_fd, HV_X86_RFLAGS);
    rflags_to_lflags(cpu);
    RIP(cpu) = rreg(cpu->mac_vcpu_fd, HV_X86_RIP);
...
{
    int i = 0;
    wreg(cpu->mac_vcpu_fd, HV_X86_RAX, RAX(cpu));
    wreg(cpu->mac_vcpu_fd, HV_X86_RBX, RBX(cpu));
    wreg(cpu->mac_vcpu_fd, HV_X86_RCX, RCX(cpu));
    wreg(cpu->mac_vcpu_fd, HV_X86_RDX, RDX(cpu));
    wreg(cpu->mac_vcpu_fd, HV_X86_RSI, RSI(cpu));
    wreg(cpu->mac_vcpu_fd, HV_X86_RDI, RDI(cpu));
    wreg(cpu->mac_vcpu_fd, HV_X86_RBP, RBP(cpu));
    wreg(cpu->mac_vcpu_fd, HV_X86_RSP, RSP(cpu));
    for (i = 8; i < 16; i++)
        wreg(cpu->mac_vcpu_fd, HV_X86_RAX + i, RRX(cpu, i));
    
    lflags_to_rflags(cpu);
    wreg(cpu->mac_vcpu_fd, HV_X86_RFLAGS, RFLAGS(cpu));
...
{
    //if (veertu_vcpu_id(cpu))
    //printf("%d, %llx: exec_instruction %s\n", veertu_vcpu_id(cpu),  RIP(cpu), decode_cmd_to_string(ins->cmd));
