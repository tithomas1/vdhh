...
#define SET_FLAGS_OSZAPC_SIZE(size, lf_carries, lf_result) { \
    ...
    cpu->lflags.result = (addr_t)(int##size##_t)(lf_result); \
    ...
    cpu->lflags.auxbits = (addr_t)(uint32_t)temp; \
}
...
#define SET_FLAGS_OSZAP_SIZE(size, lf_carries, lf_result) { \
    ...
    cpu->lflags.result = (addr_t)(int##size##_t)(lf_result); \
    addr_t delta_c = (cpu->lflags.auxbits ^ temp) & LF_MASK_CF; \
    delta_c ^= (delta_c >> 1); \
    cpu->lflags.auxbits = (addr_t)(uint32_t)(temp ^ delta_c); \
}
