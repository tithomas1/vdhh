...
static kern_return_t send_message_to_helper(usb_device_t**, int, xpc_object_t);
...
static void darwin_deref_cached_device(struct darwin_cached_device *cached_dev) {
  ...
  if (0 == cached_dev->refcount) {
    ...
    if (cached_dev->device) {
      usbi_dbg("Device not closed on dereferencing!");
...
static void darwin_hotplug_poll (void)
{
  ...
  mach_timespec_t timeout = {.tv_sec = 0, .tv_nsec = 500000};
...
static int darwin_check_configuration (struct libusb_context *ctx, struct darwin_cached_device *dev) {
  ...
  ret = get_ioregistry_value_number (dev->service, CFSTR("Preferred Configuration"),
                                     kCFNumberSInt8Type, &dev->active_config);
  return ret ? LIBUSB_SUCCESS : LIBUSB_ERROR_OTHER;
}

static int darwin_request_descriptor (usb_device_t **device, UInt8 desc, UInt8 desc_index, void *buffer, size_t buffer_size) {
  IOUSBDevRequestTO req;

  assert(desc < 16 && desc_index < 16);
  memset (buffer, 0, buffer_size);

  /* Set up request for descriptor/ */
  req.bmRequestType = USBmakebmRequestType(kUSBIn, kUSBStandard, kUSBDevice);
  req.bRequest      = kUSBRqGetDescriptor;
  req.wValue        = (desc << 8) | desc_index;
  req.wIndex        = 0;    // lang
...
#define USBDEVDESC_GET_FIELD(service, desc, name) { \
CFNumberRef number = IORegistryEntryCreateCFProperty(service, CFSTR(#name), kCFAllocatorDefault, 0); \
SInt32 val = 0; \
if (number) { \
CFNumberGetValue(number, kCFNumberSInt32Type, &val); \
CFRelease(number); } \
desc->name = val; }

static int darwin_fill_device_descriptor(io_service_t service, IOUSBDeviceDescriptorPtr desc) {
    desc->bLength = sizeof(*desc);
    desc->bDescriptorType = kUSBDeviceDesc;
    USBDEVDESC_GET_FIELD(service, desc, bcdUSB);
    USBDEVDESC_GET_FIELD(service, desc, bDeviceClass);
    USBDEVDESC_GET_FIELD(service, desc, bDeviceProtocol);
    USBDEVDESC_GET_FIELD(service, desc, bMaxPacketSize0);
    USBDEVDESC_GET_FIELD(service, desc, idVendor);
    USBDEVDESC_GET_FIELD(service, desc, idProduct);
    USBDEVDESC_GET_FIELD(service, desc, bcdDevice);
    USBDEVDESC_GET_FIELD(service, desc, iManufacturer);
    USBDEVDESC_GET_FIELD(service, desc, iProduct);
    USBDEVDESC_GET_FIELD(service, desc, iSerialNumber);
    USBDEVDESC_GET_FIELD(service, desc, bNumConfigurations);
...
static int darwin_cache_device_descriptor (struct libusb_context *ctx, struct darwin_cached_device *dev) {
  ...
  ret = darwin_fill_device_descriptor(dev->service, &dev->dev_descriptor);

  if (ret != kIOReturnSuccess) {
    /* a debug message was already printed out for this error */
    if (LIBUSB_CLASS_HUB == dev->dev_descriptor.bDeviceClass)
      usbi_dbg ("could not retrieve device descriptor %.4x:%.4x: %s (%x). skipping device",
                dev->dev_descriptor.idVendor, dev->dev_descriptor.idProduct, darwin_error_str (ret), ret);
    else
...
static int darwin_get_cached_device(struct libusb_context *ctx, io_service_t service,
  ...
  if (!get_ioregistry_value_number (service, CFSTR("sessionID"), kCFNumberSInt64Type, &sessionID))
    return LIBUSB_ERROR_NOT_FOUND;
  ...
  do {
    *cached_out = NULL;
    ...
    new_device->device = NULL;          // not created till darwin_open
    new_device->session = sessionID;
    (void) get_ioregistry_value_number (service, CFSTR("locationID"), kCFNumberSInt32Type, &new_device->location);
    ...
    if (new_device->can_enumerate) {
      snprintf(new_device->sys_path, sizeof(new_device->sys_path) - 1, "%04x-%04x-%02x-%02x",
...
static int process_new_device (struct libusb_context *ctx, io_service_t service) {
  ...
  do {
    ...
    dev->bus_number     = 0;
    dev->device_address = cached_device->location;

    ret = get_ioregistry_value_number (service, CFSTR("Device Speed"), kCFNumberSInt8Type, &devSpeed);
    if (!ret)
      return LIBUSB_ERROR_INVALID_PARAM;
...
static int darwin_open (struct libusb_device_handle *dev_handle) {
  ...
  if (0 == dpriv->open_count) {
    ...
    if (kIOReturnNotPermitted == kresult) {
      usbi_dbg("Failed to authorize the service. Error %x", kresult);
      return darwin_to_libusb(kresult);
    }

    assert(NULL == dpriv->device);
    ...
    kresult = (*dpriv->device)->GetConfiguration(dpriv->device, &dpriv->active_config);
    if (kresult != kIOReturnSuccess) {
      usbi_warn (HANDLE_CTX (dev_handle), "darwin_open_cfg: %s", darwin_error_str(kresult));
      return darwin_to_libusb (kresult);
    }

    kresult = (*(dpriv->device))->USBDeviceOpenSeize (dpriv->device);
    if (kresult != kIOReturnSuccess) {
      usbi_warn (HANDLE_CTX (dev_handle), "darwin_open: %s", darwin_error_str(kresult));
...
static int darwin_set_configuration(struct libusb_device_handle *dev_handle, int config) {
  ...
  // some devices (like smard cards) don't allow extra resets by SetConfiguration
  io_service_t usbInterfacep = 0;
  if (dpriv->active_config == config && KERN_SUCCESS == darwin_get_interface(dpriv->device, 0, &usbInterfacep)) {
    UInt8 cls = 0;
    uint32_t size = sizeof(cls);
    kresult = IORegistryEntryGetProperty(usbInterfacep, "bInterfaceClass", (char*)&cls, &size);
    IOObjectRelease(usbInterfacep);
    if (KERN_SUCCESS == kresult && 0x0B /* Smart Cards */ == cls) {
      return 0;
    }
  }
  ...
  kresult = (*(dpriv->device))->SetConfigurationV2(dpriv->device, config, false, false);
  if (kIOReturnNotPermitted == kresult) {
      if (priv->is_open)
        (*(dpriv->device))->USBDeviceClose(dpriv->device);

      char command[64];
      sprintf(command, "usbhelper setcfg 0x%x %u", dpriv->location, config);
      kresult = vsystem(command, 1);

      if (priv->is_open)
        (*(dpriv->device))->USBDeviceOpenSeize(dpriv->device);
  }
...
int darwin_get_interface (usb_device_t **darwin_device, uint8_t ifc, io_service_t *usbInterfacep) {
...
int darwin_kernel_driver_active(struct libusb_device_handle *dev_handle, int interface) {
	struct darwin_cached_device *dpriv = DARWIN_CACHED_DEVICE(dev_handle->dev);
	...
	// check the interfaces is matched
	uint64_t state = 0;
	kresult = IOServiceGetState(usbInterface, &state);
//	assert(KERN_SUCCESS == kresult);
	if ((KERN_SUCCESS == kresult) && (state & kIOServiceMatchedState)) {
		return 1;
	}

	// check the interface used by user mode clients
	io_iterator_t it;
	kresult = IORegistryEntryGetChildIterator(usbInterface, kIOServicePlane, &it);
	if (KERN_SUCCESS != kresult) {
		return LIBUSB_ERROR_OTHER;
	}
	io_object_t child = 0;
	while (( child = IOIteratorNext(it) )) {
		// TODO: check it is our IOUserClient
		break;
	}
	IOObjectRelease(it);
	if (child) {
...
int darwin_detach_kernel_driver (struct libusb_device_handle *dev_handle, int interface) {
	struct darwin_cached_device *dpriv = DARWIN_CACHED_DEVICE(dev_handle->dev);
    usb_device_t** dev = dpriv->device;
    IOReturn kresult;
...
static int darwin_claim_interface(struct libusb_device_handle *dev_handle, int iface) {
  ...
  if (kresult) {
    usbi_err (HANDLE_CTX (dev_handle), "USBInterfaceOpen: %x (%s)", kresult, darwin_error_str(kresult));
    return darwin_to_libusb (kresult);
  }
...
static int darwin_release_interface(struct libusb_device_handle *dev_handle, int iface) {
  ...
  // attach kernel driver back if requested
  if (dev_handle->auto_detach_kernel_driver) {
    kresult = darwin_attach_kernel_driver(dev_handle, iface);
  }
...
static int darwin_reset_device(struct libusb_device_handle *dev_handle) {
...
#if 0   // do not reset device actually to prevent deadloop with host operating system
  ...
  do {
    ...
    kresult = darwin_request_descriptor (dpriv->device, kUSBDeviceDesc, 0, &descriptor, sizeof (descriptor));
    if (0 != kresult) {
      usbi_dbg ("darwin/reset_device: request device descriptor failed, err 0x%x", kresult);
      return darwin_to_libusb(kresult);
    }
    ...
    for (i = 0 ; i < descriptor.bNumConfigurations ; ++i) {
      ...
      kresult = darwin_request_descriptor (dpriv->device, kUSBConfDesc, i, &configuration, sizeof (configuration));
      if (0 != kresult) {
        usbi_err("darwin/reset_device: getting configuration %d after reset failed. err 0x%x", i, kresult);
        return darwin_to_libusb (kresult);
      }

      kresult = (*(dpriv->device))->GetConfigurationDescriptorPtr (dpriv->device, i, &cached_configuration);
      if (0 != kresult) {
        usbi_err("darwin/reset_device: getting cached configuration %d after reset failed. err 0x%x", i, kresult);
        return darwin_to_libusb (kresult);
      }
  ...
  if (reenumerate) {
    ...
    return (0 == kresult) ? LIBUSB_ERROR_NOT_FOUND : darwin_to_libusb (kresult);
  }
