...
static void scsi_bus_class_init(VeertuTypeClassHold *klass, void *data)
{
...
static const VeertuTypeInfo scsi_bus_info = {
    .name = TYPE_SCSI_BUS,
    .parent = TYPE_BUS,
...
static void scsi_dma_restart_cb(void *opaque, int running, RunState state)
{
    ...
    if (!s->bh) {
        s->bh = vmx_bh_new(scsi_dma_restart_bh, s);
...
static void scsi_qdev_realize(DeviceState *qdev, Error **errp)
{
    ...
    dev->vmsentry = vmx_add_vm_change_state_handler(scsi_dma_restart_cb,
                                                     dev);
}
...
static void scsi_qdev_unrealize(DeviceState *qdev, Error **errp)
{
    ...
    if (dev->vmsentry) {
        vmx_del_vm_change_state_handler(dev->vmsentry);
    }
                                      ...
                                      const char *serial, Error **errp)
{
    ...
    Error *err = NULL;
    SCSIDevice *s;
    struct SCSIDiskState *scsi_state;

    driver = blk_is_sg(blk) ? "scsi-generic" : "scsi-disk";
    dev = qdev_create(&bus->qbus, driver);
    s = SCSI_DEVICE(dev);
    scsi_state = DO_UPCAST(struct SCSIDiskState, qdev, s);
    s->id = unit;
    
    if (bootindex > 0)
        scsi_state->qdev.conf.bootindex = bootindex;

    scsi_state->qdev.conf.blk = blk;
    blk_attach_dev(blk, s);
...
void scsi_bus_legacy_handle_cmdline(SCSIBus *bus, Error **errp)
{
    ...
    for (unit = 0; unit <= bus->info->max_target; unit++) {
        ...
        vmx_opts_loc_restore(dinfo->opts);
...
static bool scsi_target_emulate_inquiry(SCSITargetReq *r)
{
    ...
    if (r->req.lun != 0) {
        ...
        memcpy(&r->buf[8], "Veertu  ", 8);
        memcpy(&r->buf[16], "VEERTU TARGET   ", 16);
        pstrcpy((char *) &r->buf[32], 4, vmx_get_version());
    }
    return true;
}
                            ...
                            uint32_t tag, uint32_t lun, void *hba_private)
{
    ...
    req->status = -1;
    req->ops = reqops;
    veertu_notifiers_init(&req->cancel_notifiers);
    return req;
}
...
void scsi_req_complete(SCSIRequest *req, int status)
{
    ...
    veertu_notifiers_notify(&req->cancel_notifiers, req);
    scsi_req_unref(req);
}
...
{
    if (notifier) {
        veertu_notifiers_add(&req->cancel_notifiers, notifier);
    }
    if (req->io_canceled) {
        return;
    }
...
static void put_scsi_requests(QEMUFile *f, void *pv, size_t size)
{
    ...
    QTAILQ_FOREACH(req, &s->requests, next) {
        ...
        vmx_put_sbyte(f, req->retry ? 1 : 2);
        vmx_put_buffer(f, req->cmd.buf, sizeof(req->cmd.buf));
        vmx_put_be32s(f, &req->tag);
        vmx_put_be32s(f, &req->lun);
...
static int get_scsi_requests(QEMUFile *f, void *pv, size_t size)
{
    ...
    while ((sbyte = vmx_get_sbyte(f)) > 0) {
        uint8_t buf[SCSI_CMD_BUF_SIZE];
        uint32_t tag;
        uint32_t lun;
        SCSIRequest *req;

        vmx_get_buffer(f, buf, sizeof(buf));
        vmx_get_be32s(f, &tag);
        vmx_get_be32s(f, &lun);
...
static void scsi_device_class_init(VeertuTypeClassHold *klass, void *data)
{
...
static void scsi_dev_instance_init(VeertuType *obj)
{
...
static const VeertuTypeInfo scsi_device_type_info = {
...
{
    register_type_internal(&scsi_bus_info);
    register_type_internal(&scsi_device_type_info);
}
