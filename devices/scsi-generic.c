...
static void scsi_generic_save_request(QEMUFile *f, SCSIRequest *req)
{
    ...
    vmx_put_sbe32s(f, &r->buflen);
    if (r->buflen && r->req.cmd.mode == SCSI_XFER_TO_DEV) {
        assert(!r->req.sg);
        vmx_put_buffer(f, r->buf, r->req.cmd.xfer);
    }
}
...
static void scsi_generic_load_request(QEMUFile *f, SCSIRequest *req)
{
    ...
    vmx_get_sbe32s(f, &r->buflen);
    if (r->buflen && r->req.cmd.mode == SCSI_XFER_TO_DEV) {
        assert(!r->req.sg);
        vmx_get_buffer(f, r->buf, r->req.cmd.xfer);
    }
}
...
static void scsi_generic_class_initfn(VeertuTypeClassHold *klass, void *data)
{
...
static const VeertuTypeInfo scsi_generic_info = {
    .name          = "scsi-generic",
...
{
    register_type_internal(&scsi_generic_info);
}
