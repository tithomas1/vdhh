...
struct XHCIState {
    ...
    USBBus bus;
    VeertuMemArea mem;
    VeertuMemArea mem_cap;
    VeertuMemArea mem_oper;
    VeertuMemArea mem_runtime;
    VeertuMemArea mem_doorbell;
...
{
    int64_t now = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
static void xhci_mfwrap_update(XHCIState *xhci)
{
    ...
    if ((xhci->usbcmd & bits) == bits) {
        now = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
static inline uint64_t xhci_addr64(uint32_t low, uint32_t high)
{
    if (sizeof(uint64_t) == 4) {
        return low;
    } else {
        return low | (((uint64_t)high << 16) << 16);
    }
}
...
static inline void xhci_dma_read_u32s(XHCIState *xhci, uint64_t addr,
...
static inline void xhci_dma_write_u32s(XHCIState *xhci, uint64_t addr,
                                       uint32_t *buf, size_t len)
{
    int i;
...
static void xhci_er_reset(XHCIState *xhci, int v)
{
    ...
    uint64_t erstba = xhci_addr64(intr->erstba_low, intr->erstba_high);
...
static void xhci_run(XHCIState *xhci)
{
    ...
    xhci->mfindex_start = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
}
...
static void xhci_alloc_streams(XHCIEPContext *epctx, uint64_t base)
{
...
static TRBCCode xhci_enable_ep(XHCIState *xhci, unsigned int slotid,
                               unsigned int epid, uint64_t pctx,
                               uint32_t *ctx)
{
    XHCISlot *slot;
    XHCIEPContext *epctx;
    ...
    for (i = 0; i < xfer->trb_count; i++) {
        ...
        case TR_ISOCH:
            ...
            if (trb->control & TRB_TR_IDT) {
                ...
                veertu_sglist_add(&xfer->sgl, trb->addr, chunk);
            } else {
                veertu_sglist_add(&xfer->sgl, addr, chunk);
            }
            break;
        }
    }
...
err:
    veertu_sglist_destroy(&xfer->sgl);
    xhci_die(xhci);
    return -1;
}
                                     ...
                                     XHCIEPContext *epctx, uint64_t mfindex)
{
    if (xfer->mfindex_kick > mfindex) {
        timer_mod(epctx->kick_timer, vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL) +
...
static void xhci_via_challenge(XHCIState *xhci, uint64_t addr)
{
    ...
    uint32_t buf[8];
    uint32_t obuf[8];
    uint64_t paddr = xhci_mask64(addr);
                            ...
                            uint64_t val, unsigned size)
{
    ...
    case 0x1c: /* CRCR high */
        ...
        if (xhci->crcr_low & (CRCR_CA|CRCR_CS) && (xhci->crcr_low & CRCR_CRR)) {
            ...
            uint64_t base = xhci_addr64(xhci->crcr_low & ~0x3f, val);
...
static const MemAreaOps xhci_cap_ops = {
    .read = xhci_cap_read,
...
static const MemAreaOps xhci_oper_ops = {
    .read = xhci_oper_read,
...
static const MemAreaOps xhci_port_ops = {
    .read = xhci_port_read,
...
static const MemAreaOps xhci_runtime_ops = {
...
static const MemAreaOps xhci_doorbell_ops = {
...
static void usb_xhci_init(XHCIState *xhci)
{
    ...
    // apply opts
    xhci->flags |= vmx_opt_get_bool(dev->opts, "msi", true) ? (1 << XHCI_FLAG_USE_MSI) : 0;
    xhci->flags |= vmx_opt_get_bool(dev->opts, "msix", true) ? (1 << XHCI_FLAG_USE_MSI_X) : 0;
    xhci->flags |= vmx_opt_get_bool(dev->opts, "superspeed-ports-first", true) ? (1 << XHCI_FLAG_SS_FIRST) : 0;
    xhci->flags |= vmx_opt_get_bool(dev->opts, "force-pcie-endcap", false) ? (1 << XHCI_FLAG_FORCE_PCIE_ENDCAP) : 0;
    xhci->flags |= vmx_opt_get_bool(dev->opts, "streams", true) ? (1 << XHCI_FLAG_ENABLE_STREAMS) : 0;
    xhci->numintrs = vmx_opt_get_number(dev->opts, "intrs", MAXINTRS);
    xhci->numslots = vmx_opt_get_number(dev->opts, "slots", MAXSLOTS);
    xhci->numports_2 = vmx_opt_get_number(dev->opts, "p2", 4);
    xhci->numports_3 = vmx_opt_get_number(dev->opts, "p3", 4);
...
static int usb_xhci_initfn(struct PCIDevice *dev)
{
    ...
    memory_area_init(&xhci->mem, "xhci", LEN_REGS);
    memory_area_init_io(&xhci->mem_cap, VeertuTypeHold(xhci), &xhci_cap_ops, xhci,
                          "capabilities", LEN_CAP);
    memory_area_init_io(&xhci->mem_oper, VeertuTypeHold(xhci), &xhci_oper_ops, xhci,
                          "operational", 0x400);
    memory_area_init_io(&xhci->mem_runtime, VeertuTypeHold(xhci), &xhci_runtime_ops, xhci,
                          "runtime", LEN_RUNTIME);
    memory_area_init_io(&xhci->mem_doorbell, VeertuTypeHold(xhci), &xhci_doorbell_ops, xhci,
                          "doorbell", LEN_DOORBELL);

    mem_area_add_child(&xhci->mem, 0,            &xhci->mem_cap);
    mem_area_add_child(&xhci->mem, OFF_OPER,     &xhci->mem_oper);
    mem_area_add_child(&xhci->mem, OFF_RUNTIME,  &xhci->mem_runtime);
    mem_area_add_child(&xhci->mem, OFF_DOORBELL, &xhci->mem_doorbell);

    for (i = 0; i < xhci->numports; i++) {
        XHCIPort *port = &xhci->ports[i];
        uint32_t offset = OFF_OPER + 0x400 + 0x10 * i;
        port->xhci = xhci;
        memory_area_init_io(&port->mem, VeertuTypeHold(xhci), &xhci_port_ops, port,
                              port->name, 0x10);
        mem_area_add_child(&xhci->mem, offset, &port->mem);
    }
...
static void usb_xhci_exit(PCIDevice *dev)
{
    ...
    mem_are_del_child(&xhci->mem, &xhci->mem_cap);
    mem_are_del_child(&xhci->mem, &xhci->mem_oper);
    mem_are_del_child(&xhci->mem, &xhci->mem_runtime);
    mem_are_del_child(&xhci->mem, &xhci->mem_doorbell);

    for (i = 0; i < xhci->numports; i++) {
        XHCIPort *port = &xhci->ports[i];
        mem_are_del_child(&xhci->mem, &port->mem);
    }

    /* destroy msix memory region */
    if (dev->msix_table && dev->msix_pba
        && dev->msix_entry_used) {
        mem_are_del_child(&xhci->mem, &dev->msix_table_mmio);
        mem_are_del_child(&xhci->mem, &dev->msix_pba_mmio);
    }
...
static const VMStateDescription vmstate_xhci = {
    ...
    .fields = (VMStateField[]) {
        //VMSTATE_MSIX(parent, XHCIState),
        {
        .name = stringify(parent),
        .size = sizeof(PCIDevice),
        .vmsd = &msixvms,
        .flags = VMS_STRUCT,
        .offset = vmstate_offset_value(XHCIState, parent, PCIDevice),
        },
...
static void xhci_class_init(VeertuTypeClassHold *klass, void *data)
{
...
static const VeertuTypeInfo xhci_info = {
    .name          = TYPE_XHCI,
...
{
    register_type_internal(&xhci_info);
}
