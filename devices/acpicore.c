...
static const char unsigned dfl_hdr[ACPI_TABLE_HDR_SIZE - ACPI_TABLE_PFX_SIZE] =
    "VRTU\0\0\0\0\1\0"       /* sig (4), len(4), revno (1), csum (1) */
    "VEERTU  VEERTU\1\0\0\0" /* OEM id (6), table (8), revno (4) */
    "VRTU\1\0\0\0"           /* ASL compiler ID (4), version (4) */
    ;

char unsigned *acpi_tables;
size_t acpi_tables_len;

static QemuOptsList vmx_acpi_opts = {
    .name = "acpi",
    .implied_opt_name = "data",
    .head = QTAILQ_HEAD_INITIALIZER(vmx_acpi_opts.head),
    .desc = { { 0 } } /* validated with OptsVisitor */
};

void acpi_register_config(void)
{
    vmx_add_opts(&vmx_acpi_opts);
}
...
uint16_t acpi_pm1_evt_get_sts(ACPIREGS *ar)
{
    ...
    int64_t d = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
{
    ar->pm1.evt.en = val;
    vmx_system_wakeup_enable(QEMU_WAKEUP_REASON_RTC,
                              val & ACPI_BITMASK_RT_CLOCK_ENABLE);
    vmx_system_wakeup_enable(QEMU_WAKEUP_REASON_PMTIMER,
...
void acpi_pm1_evt_reset(ACPIREGS *ar)
{
    ar->pm1.evt.sts = 0;
    ar->pm1.evt.en = 0;
    vmx_system_wakeup_enable(QEMU_WAKEUP_REASON_RTC, 0);
    vmx_system_wakeup_enable(QEMU_WAKEUP_REASON_PMTIMER, 0);
}
...
static const MemAreaOps acpi_pm_evt_ops = {
...
void acpi_pm1_evt_init(ACPIREGS *ar, acpi_update_sci_fn update_sci,
                       VeertuMemArea *parent)
{
    ar->pm1.evt.update_sci = update_sci;
    memory_area_init_io(&ar->pm1.evt.io, NULL,
                          &acpi_pm_evt_ops, ar, "acpi-evt", 4);
    mem_area_add_child(parent, 0, &ar->pm1.evt.io);
}
...
{
    ACPIREGS *ar = opaque;
    vmx_system_wakeup_request(QEMU_WAKEUP_REASON_PMTIMER);
    ar->tmr.update_sci(ar);
}
...
static const MemAreaOps acpi_pm_tmr_ops = {
                      ...
                      VeertuMemArea *parent)
{
    ...
    memory_area_init_io(&ar->tmr.io,NULL ,
                          &acpi_pm_tmr_ops, ar, "acpi-tmr", 4);
    mem_area_add_child(parent, 8, &ar->tmr.io);
}
...
static const MemAreaOps acpi_pm_cnt_ops = {
...
void acpi_pm1_cnt_init(ACPIREGS *ar, VeertuMemArea *parent, uint8_t s4_val)
{
    ar->pm1.cnt.s4_val = s4_val;
    ar->wakeup.notify = acpi_notify_wakeup;
    vmx_register_wakeup_notifier(&ar->wakeup);
    memory_area_init_io(&ar->pm1.cnt.io, NULL,
                          &acpi_pm_cnt_ops, ar, "acpi-cnt", 2);
    mem_area_add_child(parent, 4, &ar->pm1.cnt.io);
}
...
void acpi_update_sci(ACPIREGS *regs, vmx_irq irq)
{
