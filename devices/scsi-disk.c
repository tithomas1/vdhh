...
static uint32_t scsi_init_iovec(SCSIDiskReq *r, size_t size)
{
    ...
    vmx_iovec_init_external(&r->qiov, &r->iov, 1);
...
static void scsi_disk_save_request(QEMUFile *f, SCSIRequest *req)
{
    ...
    vmx_put_be64s(f, &r->sector);
    vmx_put_be32s(f, &r->sector_count);
    vmx_put_be32s(f, &r->buflen);
    if (r->buflen) {
        if (r->req.cmd.mode == SCSI_XFER_TO_DEV) {
            vmx_put_buffer(f, r->iov.iov_base, r->iov.iov_len);
        } else if (!req->retry) {
...
static void scsi_disk_load_request(QEMUFile *f, SCSIRequest *req)
{
    ...
    vmx_get_be64s(f, &r->sector);
    vmx_get_be32s(f, &r->sector_count);
    vmx_get_be32s(f, &r->buflen);
    if (r->buflen) {
        scsi_init_iovec(r, r->buflen);
        if (r->req.cmd.mode == SCSI_XFER_TO_DEV) {
            vmx_get_buffer(f, r->iov.iov_base, r->iov.iov_len);
        } else if (!r->req.retry) {
            uint32_t len;
            vmx_get_be32s(f, &len);
            r->iov.iov_len = len;
...
static void scsi_do_read(void *opaque, int ret)
{
    ...
    if (r->req.sg) {
        block_acct_start(blk_get_stats(s->qdev.conf.blk), &r->acct, r->req.sg->size, BLOCK_ACCT_READ);
...
static void scsi_write_data(SCSIRequest *req)
{
    ...
    if (r->req.sg) {
        block_acct_start(blk_get_stats(s->qdev.conf.blk), &r->acct, r->req.sg->size, BLOCK_ACCT_WRITE);
        r->req.resid -= r->req.sg->size;
...
static void scsi_disk_emulate_write_same(SCSIDiskReq *r, uint8_t *inbuf)
{
    ...
    data->iov.iov_base = buf = blk_blockalign(s->qdev.conf.blk,
                                              data->iov.iov_len);
    vmx_iovec_init_external(&data->qiov, &data->iov, 1);
...
static void scsi_realize(SCSIDevice *dev, Error **errp)
{
    ...
    if (!s->version) {
        s->version = g_strdup(vmx_get_version());
    }
    if (!s->vendor) {
        s->vendor = g_strdup("Veertu");
    }
...
static void scsi_hd_realize(SCSIDevice *dev, Error **errp)
{
    ...
    s->port_wwn = 0;
    s->port_index = 0;
    s->max_unmap_size = DEFAULT_MAX_UNMAP_SIZE;
    s->max_io_size = DEFAULT_MAX_IO_SIZE;
    
    s->qdev.conf.logical_block_size = 512;
    s->qdev.conf.physical_block_size = 512;
    s->qdev.conf.min_io_size = 0;
    s->qdev.conf.opt_io_size = 0;
    s->qdev.conf.discard_granularity = -1;

    if (!s->qdev.conf.blk) {
        snprintf(name, 256, "disk.%d", disk_num++);
        s->qdev.conf.blk= blk_by_name(get_current_conf_name());;
        blk_attach_dev(s->qdev.conf.blk, dev);
    }
...
static void scsi_disk_realize(SCSIDevice *dev, Error **errp)
{
    ...
    s = DO_UPCAST(SCSIDiskState, qdev, dev);
    s->features = 0;
    s->wwn = 0;
    s->port_index = 0;
    s->port_wwn = 0;
...
static void scsi_hd_class_initfn(VeertuTypeClassHold *klass, void *data)
{
...
static const VeertuTypeInfo scsi_hd_info = {
    .name          = "scsi-hd",
...
static void scsi_cd_class_initfn(VeertuTypeClassHold *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    SCSIDeviceClass *sc = SCSI_DEVICE_CLASS(klass);
...
static const VeertuTypeInfo scsi_cd_info = {
    .name          = "scsi-cd",
    .parent        = TYPE_SCSI_DEVICE,
    .instance_size = sizeof(SCSIDiskState),
...
static void scsi_block_class_initfn(VeertuTypeClassHold *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    SCSIDeviceClass *sc = SCSI_DEVICE_CLASS(klass);
...
static const VeertuTypeInfo scsi_block_info = {
    .name          = "scsi-block",
    .parent        = TYPE_SCSI_DEVICE,
    .instance_size = sizeof(SCSIDiskState),
...
static void scsi_disk_class_initfn(VeertuTypeClassHold *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    SCSIDeviceClass *sc = SCSI_DEVICE_CLASS(klass);
...
static const VeertuTypeInfo scsi_disk_info = {
    .name          = "scsi-disk",
    .parent        = TYPE_SCSI_DEVICE,
    .instance_size = sizeof(SCSIDiskState),
...
{
    register_type_internal(&scsi_hd_info);
    register_type_internal(&scsi_cd_info);
#ifdef __linux__
    register_type_internal(&scsi_block_info);
#endif
    register_type_internal(&scsi_disk_info);
}
