...
#include "emudma.h"
#include "ideinternal.h"
#include <devices/idepci.h>
...
static void map_page(VeertuAddressSpace *as, uint8_t **ptr, uint64_t addr,
                     uint32_t wanted)
{
    hwaddr len = wanted;

    if (*ptr) {
        dma_memory_unmap(as, *ptr, len, 1, len);
    }

    *ptr = dma_memory_map(as, addr, &len, 1);
    if (len < wanted) {
        dma_memory_unmap(as, *ptr, len, 1, len);
        *ptr = NULL;
    }
...
static const MemAreaOps ahci_mem_ops = {
    .read = ahci_mem_read,
...
static const MemAreaOps ahci_idp_ops = {
    .read = ahci_idp_read,
...
static void ahci_reset_port(AHCIState *s, int port)
{
    ...
    for (i = 0; i < AHCI_MAX_CMDS; i++) {
        ...
        veertu_sglist_destroy(&ncq_tfs->sglist);
        ncq_tfs->used = 0;
    }
...
static int ahci_populate_sglist(AHCIDevice *ad, VeertuSGList *sglist,
                                AHCICmdHdr *cmd, int64_t limit, uint64_t offset)
{
    ...
    uint64_t prdt_len = (prdtl * sizeof(AHCI_SG));
    uint64_t real_prdt_len = prdt_len;
    uint8_t *prdt;
    int i;
    int r = 0;
    uint64_t sum = 0;
    ...
    if (prdtl > 0) {
        ...
        veertu_sglist_init(sglist, qbus->parent, (prdtl - off_idx),
                         ad->hba->as);
        veertu_sglist_add(sglist, le64_to_cpu(tbl[off_idx].addr) + off_pos,
                        MIN(prdt_tbl_entry_size(&tbl[off_idx]) - off_pos,
                            limit));

        for (i = off_idx + 1; i < prdtl && sglist->size < limit; i++) {
            veertu_sglist_add(sglist, le64_to_cpu(tbl[i].addr),
...
static void execute_ncq_command(NCQTransferState *ncq_tfs)
{
    ...
    case READ_FPDMA_QUEUED:
        ...
        block_acct_start(ide_state->blk, &ncq_tfs->acct,
...
out:
    dma_memory_unmap(s->as, cmd_fis, cmd_len, 1,
                     cmd_len);
...
static void ahci_cmd_done(IDEDMA *dma)
{
    ...
    if (!ad->check_bh) {
        ...
        ad->check_bh = vmx_bh_new(ahci_check_cmd_bh, ad);
        vmx_bh_schedule(ad->check_bh);
    }
}
...
void ahci_init(AHCIState *s, DeviceState *qdev, VeertuAddressSpace *as, int ports)
{
    vmx_irq *irqs;
    int i;
    ...
    memory_area_init_io(&s->mem, VeertuTypeHold(qdev), &ahci_mem_ops, s,
                          "ahci", AHCI_MEM_BAR_SIZE);
    memory_area_init_io(&s->idp, VeertuTypeHold(qdev), &ahci_idp_ops, s,
                          "ahci-idp", 32);

    irqs = vmx_allocate_irqs(ahci_irq_set, s, s->ports);
...
static void sysbus_ahci_class_init(VeertuTypeClassHold *klass, void *data)
{
...
static const VeertuTypeInfo sysbus_ahci_info = {
...
{
    register_type_internal(&sysbus_ahci_info);
}
