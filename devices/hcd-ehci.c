...
#define MAX_CPAGE        5
#define BUFF_SIZE        MAX_CPAGE*4096   // Max bytes to transfer per transaction
...
#define SET_LAST_RUN_CLOCK(s) \
    (s)->last_run_ns = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
                            ...
                            uint64_t val, unsigned size)
{
    ...
    *portsc &= val | ~PORTSC_PED;
    ...
    if (!(val & PORTSC_PRESET) &&(*portsc & PORTSC_PRESET)) {
         ...
         */
        if (dev && dev->attached && (dev->speedmask & (USB_SPEED_MASK_LOW | USB_SPEED_MASK_FULL | USB_SPEED_MASK_HIGH))) {
            val |= PORTSC_PED;
        }
    }
...
static int ehci_init_transfer(EHCIPacket *p)
{
    ...
    veertu_sglist_init(&p->sgl, p->queue->ehci->device, 5, p->queue->ehci->as);

    while (bytes > 0) {
        if (cpage >= MAX_CPAGE) {
            fprintf(stderr, "cpage out of range (%d), %x\n", cpage, p->qtd.bufptr[cpage]);
            usb_errors++;
            if (usb_errors > 100)
                return -1;
            break;
        ...
        veertu_sglist_add(&p->sgl, page, plen);
        bytes -= plen;
    }
    return 0;
}
                            ...
                            uint32_t addr)
{
    ...
    for(i = 0; i < 8; i++) {
        if (itd->transact[i] & ITD_XACT_ACTIVE) {
            ...
            veertu_sglist_init(&ehci->isgl, ehci->device, 2, ehci->as);
            if (off + len > 4096) {
                /* transfer crosses page border */
                uint32_t len2 = off + len - 4096;
                uint32_t len1 = len - len2;
                veertu_sglist_add(&ehci->isgl, ptr1 + off, len1);
                veertu_sglist_add(&ehci->isgl, ptr2, len2);
            } else {
                veertu_sglist_add(&ehci->isgl, ptr1 + off, len);
            }
            ...
            veertu_sglist_destroy(&ehci->isgl);
...
static void ehci_update_frindex(EHCIState *ehci, int uframes)
{
    ...
    for (i = 0; i < MIN(uframes, 0x4000 * 2); i++) {
        ehci->frindex++;
...
static void ehci_frame_timer(void *opaque)
{
    ...
    t_now = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
static const MemAreaOps ehci_mmio_caps_ops = {
    .read = ehci_caps_read,
...
static const MemAreaOps ehci_mmio_opreg_ops = {
...
static const MemAreaOps ehci_mmio_port_ops = {
    .read = ehci_port_read,
...
void usb_ehci_realize(EHCIState *s, DeviceState *dev, Error **errp)
{
    ...
    for (i = 0; i < s->portnr; i++) {
        usb_register_port(&s->bus, &s->ports[i], s, i, &ehci_port_ops,
            USB_SPEED_MASK_LOW | USB_SPEED_MASK_FULL | USB_SPEED_MASK_HIGH /* simulate RMH presence*/);
        s->ports[i].dev = 0;
    }

    s->frame_timer = timer_new_ns(QEMU_CLOCK_VIRTUAL, ehci_frame_timer, s);
    s->async_bh = vmx_bh_new(ehci_frame_timer, s);
    s->device = dev;

    vmx_register_reset(ehci_reset, s);
    s->vmstate = vmx_add_vm_change_state_handler(usb_ehci_vm_state_change, s);
}
...
void usb_ehci_unrealize(EHCIState *s, DeviceState *dev, Error **errp)
{
    ...
    if (s->async_bh) {
        vmx_bh_delete(s->async_bh);
    }

    ehci_queues_rip_all(s, 0);
    ehci_queues_rip_all(s, 1);

    mem_are_del_child(&s->mem, &s->mem_caps);
    mem_are_del_child(&s->mem, &s->mem_opreg);
    mem_are_del_child(&s->mem, &s->mem_ports);

    usb_bus_release(&s->bus);

    if (s->vmstate) {
        vmx_del_vm_change_state_handler(s->vmstate);
    }
}
...
void usb_ehci_init(EHCIState *s, DeviceState *dev)
{
    ...
    memory_area_init(&s->mem, "ehci", MMIO_SIZE);
    memory_area_init_io(&s->mem_caps, VeertuTypeHold(dev), &ehci_mmio_caps_ops, s,
                          "capabilities", CAPA_SIZE);
    memory_area_init_io(&s->mem_opreg, VeertuTypeHold(dev), &ehci_mmio_opreg_ops, s,
                          "operational", s->portscbase);
    memory_area_init_io(&s->mem_ports, VeertuTypeHold(dev), &ehci_mmio_port_ops, s,
                          "ports", 4 * s->portnr);

    mem_area_add_child(&s->mem, s->capsbase, &s->mem_caps);
    mem_area_add_child(&s->mem, s->opregbase, &s->mem_opreg);
    mem_area_add_child(&s->mem, s->opregbase + s->portscbase,
                                &s->mem_ports);
}
