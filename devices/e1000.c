...
#define E1000(obj) obj
#define E1000_DEVICE_CLASS(klass) klass
#define E1000_DEVICE_GET_CLASS(obj) ((VeertuType *)obj)->class
...
set_phy_ctrl(E1000State *s, int index, uint16_t val)
{
    ...
    if (have_autoneg(s) && (val & MII_CR_RESTART_AUTO_NEG)) {
        ...
        timer_mod(s->autoneg_timer,
                  vmx_clock_get_ms(QEMU_CLOCK_VIRTUAL) + 500);
    }
}
...
set_interrupt_cause(E1000State *s, int index, uint32_t val)
{
    ...
    if (!s->mit_irq_level && pending_ints) {
        ...
        if (s->compat_flags & E1000_FLAG_MIT) {
            ...
            if (mit_delay) {
                s->mit_timer_on = 1;
                timer_mod(s->mit_timer, vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL) +
                          mit_delay * 256);
            }
            s->mit_ide = 0;
        }
...
{
    E1000State *s = opaque;
    if (!vmx_get_queue(s->nic)->link_down) {
        e1000_link_up(s);
        s->phy_reg[PHY_LP_ABILITY] |= MII_LPAR_LPACK;
        s->phy_reg[PHY_STATUS] |= MII_SR_AUTONEG_COMPLETE;
...
static void e1000_reset(void *opaque)
{
    ...
    if (vmx_get_queue(d->nic)->link_down) {
        e1000_link_down(d);
    }
    ...
    vmx_format_nic_info_str(vmx_get_queue(d->nic), macaddr);
}
...
putsum(uint8_t *data, uint32_t n, uint32_t sloc, uint32_t css, uint32_t cse)
{
    ...
    if (cse && cse < n)
        n = cse + 1;
    if (sloc < n-1) {
        sum = ip_checksum(data+css, n-css);
        stw_be_p(data + sloc, sum);
    }
}
...
{
    NetClientState *nc = vmx_get_queue(s->nic);
    if (s->phy_reg[PHY_CTRL] & MII_CR_LOOPBACK) {
        nc->info->receive(nc, buf, size);
    } else {
        vmx_send_packet(nc, buf, size);
    }
}
...
static uint32_t
txdesc_writeback(E1000State *s, uint64_t base, struct e1000_tx_desc *dp)
{
    PCIDevice *d = PCI_DEVICE(s);
...
{
    E1000State *s = vmx_get_nic_opaque(nc);
...
mac_writereg(E1000State *s, int index, uint32_t val)
{
    ...
    if (index == RA + 1) {
        ...
        vmx_format_nic_info_str(vmx_get_queue(s->nic), (uint8_t *)macaddr);
    }
}
...
static const MemAreaOps e1000_mmio_ops = {
...
static const MemAreaOps e1000_io_ops = {
    .read = e1000_io_read,
...
static const VMStateDescription vmstate_e1000 = {
    ...
    .fields = (VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent, E1000State),
...
e1000_mmio_setup(E1000State *d)
{
    ...
    memory_area_init_io(&d->mmio, VeertuTypeHold(d), &e1000_mmio_ops, d,
                          "e1000-mmio", PNPMMIO_SIZE);
    memory_area_init_io(&d->io, VeertuTypeHold(d), &e1000_io_ops, d, "e1000-io", IOPORT_SIZE);
}
...
static int pci_e1000_init(PCIDevice *pci_dev)
{
    ...
    memcpy((void *)&d->conf.macaddr, /*(void *)&d->conf.macaddr*/&current_nd->macaddr, sizeof(d->conf.macaddr));
    d->conf.peers.ncs[dev_index++] = current_nd->netdev;
    d->conf.peers.queues = dev_index;
    ...
    vmx_macaddr_default_if_unset(&d->conf.macaddr);
    ...
    d->nic = vmx_new_nic(&net_e1000_info, &d->conf,
                          get_typename(VeertuTypeHold(d)), dev->id, d);
...
static void e1000_class_init(VeertuTypeClassHold *klass, void *data)
{
...
static void e1000_instance_init(VeertuType *obj)
{
...
static const VeertuTypeInfo e1000_base_info = {
...
static const VeertuTypeInfo e1000_default_info = {
    .name          = "e1000",
...
void e1000_register_types(void)
{
    ...
    register_type_internal(&e1000_base_info);
    ...
    register_type_internal(&e1000_default_info);
}