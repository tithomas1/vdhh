...
static inline int get_dwords(OHCIState *ohci,
                             uint64_t addr, uint32_t *buf, int num)
{
    int i;
...
static inline int get_words(OHCIState *ohci,
                            uint64_t addr, uint16_t *buf, int num)
{
    int i;
...
static inline int ohci_read_ed(OHCIState *ohci,
                               uint64_t addr, struct ohci_ed *ed)
{
    return get_dwords(ohci, addr, (uint32_t *)ed, sizeof(*ed) >> 2);
}

static inline int ohci_read_td(OHCIState *ohci,
                               uint64_t addr, struct ohci_td *td)
{
    return get_dwords(ohci, addr, (uint32_t *)td, sizeof(*td) >> 2);
}

static inline int ohci_read_iso_td(OHCIState *ohci,
                                   uint64_t addr, struct ohci_iso_td *td)
{
...
static inline int ohci_read_hcca(OHCIState *ohci,
                                 uint64_t addr, struct ohci_hcca *hcca)
{
...
{
    ohci->sof_time = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
static uint32_t ohci_get_frame_remaining(OHCIState *ohci)
{
     ...
     * set already.
     */
    tks = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL) - ohci->sof_time;
...
static const MemAreaOps ohci_mem_ops = {
    .read = ohci_mem_read,
...
static int usb_ohci_init(OHCIState *ohci, DeviceState *dev,
                         int num_ports, uint64_t localmem_base,
                         ...
                         VeertuAddressSpace *as)
{
    ...
    memory_area_init_io(&ohci->mem, VeertuTypeHold(dev), &ohci_mem_ops,
                          ohci, "ohci", 256);
    ohci->localmem_base = localmem_base;

    ohci->name = get_typename(VeertuTypeHold(dev));
    usb_packet_init(&ohci->usb_packet);

    ohci->async_td = 0;
    vmx_register_reset(ohci_reset, ohci);

    return 0;
}

#define TYPE_PCI_OHCI "pci-ohci"
#define PCI_OHCI(obj) obj
...
static void ohci_die(OHCIState *ohci)
{
    ...
    ohci_bus_stop(ohci);
    pci_set_word(dev->parent.config + PCI_STATUS,
...
static const VMStateDescription vmstate_ohci = {
    ...
    .minimum_version_id = 1,
    .fields = (VMStateField[]) {
        VMSTATE_PCI_DEVICE(parent, OHCIPCIState),
        VMSTATE_STRUCT(state, OHCIPCIState, 1, vmstate_ohci_state, OHCIState),
        VMSTATE_END_OF_LIST()
    }
};

static void ohci_pci_class_init(VeertuTypeClassHold *klass, void *data)
{
...
static const VeertuTypeInfo ohci_pci_info = {
    .name          = TYPE_PCI_OHCI,
...
static void ohci_sysbus_class_init(VeertuTypeClassHold *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
...
static const VeertuTypeInfo ohci_sysbus_info = {
...
{
    register_type_internal(&ohci_pci_info);
    register_type_internal(&ohci_sysbus_info);
}
