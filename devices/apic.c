...
static int get_highest_priority_irr(APICCommonState *s)
{
...
static void apic_sync_vapic(APICCommonState *s, int sync_type)
{
    ...
    if (sync_type & (SYNC_TO_VAPIC | SYNC_ISR_IRR_TO_VAPIC)) {
        ...
        if (sync_type & SYNC_TO_VAPIC) {
            assert(vmx_cpu_is_self(GETCPU(s->cpu)));
...
static void apic_local_deliver(APICCommonState *s, int vector)
{
    ...
    switch ((lvt >> 8) & 7) {
    case APIC_DM_SMI:
        cpu_interrupt(GETCPU(s->cpu), CPU_INTERRUPT_SMI);
        break;

    case APIC_DM_NMI:
        cpu_interrupt(GETCPU(s->cpu), CPU_INTERRUPT_NMI);
        break;

    case APIC_DM_EXTINT:
        cpu_interrupt(GETCPU(s->cpu), CPU_INTERRUPT_HARD);
        break;
    ...
    switch (delivery_mode) {
        ...
        case APIC_DM_SMI:
            foreach_apic(apic_iter, deliver_bitmask,
                cpu_interrupt(GETCPU(apic_iter->cpu), CPU_INTERRUPT_SMI)
            );
            return;

        case APIC_DM_NMI:
            foreach_apic(apic_iter, deliver_bitmask,
                cpu_interrupt(GETCPU(apic_iter->cpu), CPU_INTERRUPT_NMI)
            );
            return;
        ...
        case APIC_DM_INIT:
            ...
            foreach_apic(apic_iter, deliver_bitmask,
                         cpu_interrupt(GETCPU(apic_iter->cpu),
                                       CPU_INTERRUPT_INIT)
            );
            return;
...
/* signal the GETCPU if an irq is pending */
static void apic_update_irq(APICCommonState *s)
{
    CPUState *cpu;
...
static void apic_startup(APICCommonState *s, int vector_num)
{
    ...
    cpu_interrupt(GETCPU(s->cpu), CPU_INTERRUPT_SIPI);
}
...
void apic_sipi(DeviceState *dev)
{
    ...
    cpu_reset_interrupt(GETCPU(s->cpu), CPU_INTERRUPT_SIPI);
...
static uint32_t apic_get_current_count(APICCommonState *s)
{
    int64_t d;
    uint32_t val;
    d = (vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL) - s->initial_count_load_time) >>
        s->count_shift;
...
static void apic_mem_writel(void *opaque, hwaddr addr, uint32_t val)
{
    ...
    if (addr > 0xfff || !index) {
         ...
         * APIC is connected directly to the GETCPU.
    ...
    case 0x32 ... 0x37:
        {
            ...
            s->lvt[n] = val;
            if (n == APIC_LVT_TIMER) {
                apic_timer_update(s, vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL));
    ...
    case 0x38:
        s->initial_count = val;
        s->initial_count_load_time = vmx_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
{
    apic_mem_writel(opaque, addr, val);
}
...
static const MemAreaOps apic_io_ops = {
    .old_mmio = {
...
static void apic_realize(DeviceState *dev, Error **errp)
{
    ...
    memory_area_init_io(&s->io_memory, VeertuTypeHold(s), &apic_io_ops, s, "apic-msi",
                          APIC_SPACE_SIZE);
...
static void apic_class_init(VeertuTypeClassHold *klass, void *data)
{
    APICCommonClass *k = APIC_COMMON_CLASS(klass);

    k->realize = apic_realize;
    k->set_base = apic_set_base;
    k->get_highest_priority_irr = get_highest_priority_irr;
...
static const VeertuTypeInfo apic_info = {
    .name          = "apic",
...
{
    register_type_internal(&apic_info);
}
