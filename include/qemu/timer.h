 ...
 */
static inline int64_t vmx_clock_get_ms(QEMUClockType type)
{
    return vmx_clock_get_ns(type) / SCALE_MS;
}
 ...
 */
static inline int64_t vmx_clock_get_us(QEMUClockType type)
{
    return vmx_clock_get_ns(type) / SCALE_US;
}

/**
 * vmx_clock_has_timers:
 * @type: the clock type
 *
 ...
 * to interrupt any poll using vmx_notify/aio_notify
 * etc.
 *
...
/**
 * vmx_clock_get_main_loop_timerlist:
 * @type: the clock type
 *
 ...
 * list linked to the clock, or vmx_notify() if none.
 */
...
/**
 * vmx_clock_register_reset_notifier:
 * @type: the clock type
...
/**
 * vmx_clock_unregister_reset_notifier:
 * @type: the clock type
 * @notifier: the notifier function
 *
 ...
 * list is modified. If @cb is specified as null, vmx_notify()
 * is used instead.
 */
 ...
 */
static inline int64_t vmx_soonest_timeout(int64_t timeout1, int64_t timeout2)
{
...
/*******************************************/
/* host GETCPU ticks (if available) */
...
{
    int val;
    __asm__ __volatile__ ("mfctl %%cr16, %0" : "=r"(val));
    return val;
}
...
static inline int64_t cpu_get_real_ticks (void)
{
#if defined(_LP64)
    uint64_t        rval;
    __asm__ __volatile__("rd %%tick,%0" : "=r"(rval));
    return rval;
#else
    ...
    __asm__ __volatile__("rd %%tick,%%g1; srlx %%g1,32,%0; mov %%g1,%1"
...
#else
/* The host GETCPU doesn't have an easily accessible cycle counter.
...
extern int64_t vmx_time, vmx_time_start;
