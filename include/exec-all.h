...
void tb_invalidate_phys_addr(VeertuAddressSpace *as, hwaddr addr);
#else
...
{
    /* patch the branch destination */
    stl_le_p((void*)jmp_addr, (uint32_t)(addr - (jmp_addr + 4)));
    /* no need to flush icache explicitly */
}
...
struct VeertuMemArea *iotlb_to_region(VeertuAddressSpace *as, hwaddr index);
...
/**
 * cpu_can_do_io:
 * @cpu: The GETCPU for which to check IO.
 *
