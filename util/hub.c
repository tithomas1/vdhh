                               ...
                               const uint8_t *buf, size_t len)
{
    ...
    QLIST_FOREACH(port, &hub->ports, next) {
        ...
        vmx_send_packet(&port->nc, buf, len);
    }
    return len;
}
                                   ...
                                   const struct iovec *iov, int iovcnt)
{
    ...
    QLIST_FOREACH(port, &hub->ports, next) {
        ...
        vmx_sendv_packet(&port->nc, iov, iovcnt);
    }
    return len;
}
...
static int net_hub_port_can_receive(NetClientState *nc)
{
    ...
    QLIST_FOREACH(port, &hub->ports, next) {
        ...
        if (vmx_can_send_packet(&port->nc)) {
            return 1;
        }
    }
...
static NetHubPort *net_hub_port_new(NetHub *hub, const char *name)
{
    ...
    nc = vmx_new_net_client(&net_hub_port_info, NULL, "hub", name);
 ...
 * Find a specific client on a hub
 */
NetClientState *net_hub_find_client_by_type(NetClientState *nc, int type)
{
    NetHub *hub;
    NetHubPort *port;
    NetClientState *peer;
    ...
    QLIST_FOREACH(hub, &hubs, next) {
        if (hub->id == source_port->hub->id) {
            QLIST_FOREACH(port, &hub->ports, next) {
                peer = port->nc.peer;
                
                if (peer && peer->info->type == type) {
                    return peer;
                }
            }
        }
...
bool net_hub_flush(NetClientState *nc)
{
    ...
    QLIST_FOREACH(port, &source_port->hub->ports, next) {
        if (port != source_port) {
            ret += vmx_net_queue_flush(port->nc.incoming_queue);
        }
    }
