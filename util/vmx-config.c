...
QemuOpts *vmx_find_opts_singleton(const char *group)
{
    ...
    list = vmx_find_opts(group);
    assert(list);
    opts = vmx_opts_find(list, NULL);
    if (!opts) {
        opts = vmx_opts_create(list, NULL, 0, &error_abort);
    }
    return opts;
}
...
int vmx_set_option(const char *str)
{
    ...
    if (vmx_opt_set(opts, arg, str+offset+1) == -1) {
        return -1;
    }
    return 0;
}
...
static int config_write_opts(QemuOpts *opts, void *opaque)
{
    ...
    vmx_opt_foreach(opts, config_write_opt, data, 0);
    fprintf(data->fp, "\n");
    return 0;
}
...
void vmx_config_write(FILE *fp)
{
    ...
    fprintf(fp, "# vmx config file\n\n");
    for (i = 0; lists[i] != NULL; i++) {
        data.list = lists[i];
        vmx_opts_foreach(data.list, config_write_opts, &data, 0);
    }
}
...
int vmx_config_parse(FILE *fp, QemuOptsList **lists, const char *fname)
{
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (sscanf(line, "[%63s \"%63[^\"]\"]", group, id) == 2) {
            ...
            opts = vmx_opts_create(list, id, 1, NULL);
            continue;
        }
        ...
        if (sscanf(line, " %63s = \"%1023[^\"]\"", arg, value) == 2) {
            ...
            if (vmx_opt_set(opts, arg, value) != 0) {
                goto out;
            }
            continue;
        }
...
int vmx_read_config_file(const char *filename)
{
    ...
    ret = vmx_config_parse(f, vm_config_groups, filename);
    fclose(f);
                                       ...
                                       Error **errp)
{
    ...
    subopts = vmx_opts_create(opts, NULL, 0, &local_err);
    if (local_err) {
        error_propagate(errp, local_err);
        goto out;
    }

    vmx_opts_absorb_qdict(subopts, subqdict, &local_err);
    if (local_err) {
        error_propagate(errp, local_err);
        goto out;
    }
    ...
    if (enum_size) {
        ...
        QLIST_FOREACH_ENTRY(list, list_entry) {
            ...
            subopts = vmx_opts_create(opts, opt_name, 1, &local_err);
            g_free(opt_name);
            if (local_err) {
                error_propagate(errp, local_err);
                goto out;
            ...
            vmx_opts_absorb_qdict(subopts, section, &local_err);
            if (local_err) {
                error_propagate(errp, local_err);
                vmx_opts_del(subopts);
                goto out;
