...
static void put_fpreg(QEMUFile *f, void *opaque, size_t size)
{
    ...
    uint64_t mant;
    uint16_t exp;
    /* we save the real GETCPU data (in case of MMX usage only 'mant'
       contains the MMX register */
    vmx_put_be64s(f, &mant);
    vmx_put_be16s(f, &exp);
}
...
VMStateDescription vmstate_x86_cpu = {
    ...
    .fields = (VMStateField[]) {
        ...
        VMSTATE_UINT32_TEST(parent.halted, X86CPU, version_is_5),
